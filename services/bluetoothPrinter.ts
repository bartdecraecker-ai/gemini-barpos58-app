...
export class BluetoothPrinterService {
  ...
  private readonly COMMANDS = {
    ...
    FEED_CUT: new Uint8Array([0x1d, 0x56, 0x00]), // GS V 0
  ...
  private readonly CHUNK_DELAY_MS = 70;
  ...
  async connect(): Promise<boolean> {
    try {
      ...
      // Prefer writeWithoutResponse, fallback to normal write
      this.characteristic =
        characteristics.find(
          (c: any) => c.properties.writeWithoutResponse
        ) ||
        characteristics.find(
          (c: any) => c.properties.write
        );
  ...
  private lineBytes(
    ...
    if (align === "CENTER") {
      parts.push(this.COMMANDS.ALIGN_CENTER);
    } else if (align === "RIGHT") {
      parts.push(this.COMMANDS.ALIGN_RIGHT);
    } else {
      parts.push(this.COMMANDS.ALIGN_LEFT);
    }

    if (bold) {
      parts.push(this.COMMANDS.BOLD_ON);
    }

    parts.push(this.enc(text + "\n"));

    if (bold) {
      parts.push(this.COMMANDS.BOLD_OFF);
    }
  ...
  /**
   * Chunked writing is critical for reliability on Android BLE stacks
   * & low-cost printers.
   */
  private async writeChunked(
    data: Uint8Array,
    chunkSize = this.CHUNK_SIZE,
    delayMs = this.CHUNK_DELAY_MS
  ) {
  ...
  /**
   * Builds a full print job buffer
   * with reset + trailing feed/cut.
   */
  private async printJob(
    lines: Uint8Array[],
    options?: {
      cut?: boolean;
    ...
    bytes.push(
      new Uint8Array(
        Array(extraFeedLines).fill(0x0a)
      )
    );

    // Optional cutter
    if (cut) {
      bytes.push(this.COMMANDS.FEED_CUT);
    }
  ...
  async printReceipt(
    ...
    lines.push(
      this.lineBytes(
        `Datum: ${new Date(t.timestamp).toLocaleString("nl-NL")}`,
        "LEFT"
      )
    );
    ...
    // Right column ~10 chars

    for (const item of t.items) {
      const left =
        `${item.quantity}x ${String(item.name || "").substring(0, 16)}`
          .padEnd(20);
  ...
  async printSessionReport(
    ...
    lines.push(
      this.lineBytes(
        `Datum: ${new Date(s.startTime).toLocaleDateString("nl-NL")}`,
        "LEFT"
      )
    );
    ...
    if (s.summary?.productSales) {
      ...
      for (
        const [name, qty]
        of Object.entries(s.summary.productSales)
      ) {
        const left =
          String(name)
            .substring(0, 22)
    ...
    if (s.summary) {
      lines.push(
        this.lineBytes(
          `Tickets:   ${s.summary.transactionCount}`,
          "LEFT"
        )
      );
      ...
      lines.push(
        this.lineBytes(
          `OMZET:     EUR ${s.summary.totalSales.toFixed(2)}`,
          "LEFT",
          true
        )
      );
      ...
      lines.push(
        this.lineBytes(
          `CONTANT:   EUR ${s.summary.cashTotal.toFixed(2)}`,
          "LEFT"
        )
      );
      ...
      lines.push(
        this.lineBytes(
          `KAART:     EUR ${s.summary.cardTotal.toFixed(2)}`,
          "LEFT"
        )
      );
      ...
      lines.push(
        this.lineBytes(
          `Begin Kas: EUR ${s.startCash.toFixed(2)}`,
          "LEFT"
        )
      );
      ...
      if (s.endCash !== undefined) {
        lines.push(
          this.lineBytes(
            `Geteld:    EUR ${s.endCash.toFixed(2)}`,
            "LEFT"
          )
        );
    ...
    // If printer has a cutter, set cut:true.
    await this.printJob(
      lines,
      {
        cut: false,
