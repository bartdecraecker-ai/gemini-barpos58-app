{/* Action row for session item */}
                  <div className="flex justify-end gap-3 mt-5 pt-4 border-t border-slate-50">
                    <button 
                      ...
                      className="flex items-center gap-2 px-4 py-2 bg-slate-50 text-slate-600 rounded-xl font-bold text-[10px] uppercase hover:bg-indigo-50 hover:text-indigo-600 transition-all font-bold"
                    >
                      Rapport
                    </button>
                    <button 
                      onClick={() => deleteSessionFromHistory(s.id)} 
                      className="flex items-center gap-2 px-4 py-2 bg-rose-50 text-rose-600 rounded-xl font-bold text-[10px] uppercase hover:bg-rose-100 transition-all font-bold"
        ...
        {activeTab === 'SETTINGS' && (
          <div className="h-full overflow-y-auto p-6 space-y-6 pb-24 custom-scrollbar">
            <h2 className="text-2xl font-black tracking-tighter font-bold">Instellingen & Beheer</h2>

            {/* Product Management */}
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider font-bold">Producten (Max 10)</h3>
                <button 
                  onClick={() => exportData('PRODUCTS')} 
                  className="flex items-center gap-1.5 text-[10px] font-bold text-indigo-500 bg-indigo-50 px-3 py-1.5 rounded-full hover:bg-indigo-100 transition-colors font-bold"
                >
                ...
                {products.map(p => (
                  <div key={p.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl border border-slate-100">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-xs text-slate-800 font-bold">{p.name}</div>
            ...
            {/* Staff Management */}
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 space-y-4">
              <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider font-bold">Medewerkers</h3>
              <div className="flex gap-2">
                ...
                <button 
                  onClick={addStaff} 
                  className="bg-indigo-500 text-white px-4 rounded-xl font-bold text-xs uppercase shadow-sm active:scale-95 transition-all font-bold"
                >
                  <Plus size={16} />
                </button>
              </div>
            ...
            {/* Cloud Config & Reset */}
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 space-y-4">
              <h3 className="font-bold text-slate-800 text-sm uppercase tracking-wider font-bold">Systeem & Synchronisatie</h3>
              <div className="space-y-3">
                <button 
                  onClick={handleResetToDefaults} 
                  className="w-full bg-amber-50 text-amber-700 py-3.5 rounded-2xl font-bold text-xs uppercase border border-amber-200 hover:bg-amber-100 transition-colors flex items-center justify-center gap-2 font-bold"
                >
      ...
      {editingProduct && (
        <div className="fixed inset-0 z-[600] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white rounded-[2.5rem] p-6 max-w-sm w-full space-y-4 shadow-2xl animate-in zoom-in-95">
            <div className="flex justify-between items-center">
              <h3 className="font-black text-lg tracking-tight font-bold">Product Bewerken</h3>
              <button onClick={() => setEditingProduct(null)} className="text-slate-400 hover:text-slate-600"><X size={20} /></button>
            </div>
            <div className="space-y-3">
              <div>
                <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block font-bold">Naam</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block font-bold">Prijs (€)</label>
                <input 
                  type="number" 
                  step="0.01" 
              ...
              <div>
                <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block font-bold">Stock</label>
                <input 
                  type="number" 
            ...
            <button 
              ...
              className="w-full bg-indigo-500 text-white py-3.5 rounded-xl font-bold uppercase text-xs shadow-lg active:scale-95 transition-all font-bold"
            >
              Opslaan
            </button>
          </div>
      ...
      {showSalesmanSelection && (
        <div className="fixed inset-0 z-[600] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white rounded-[2.5rem] p-6 max-w-xs w-full space-y-4 shadow-2xl text-center animate-in zoom-in-95">
            <h3 className="font-black text-lg tracking-tight font-bold">Kies Medewerker</h3>
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {(company.salesmen || []).map(name => (
                <button 
                  ...
                  className="w-full p-3 bg-slate-50 hover:bg-indigo-50 hover:text-indigo-600 rounded-xl font-bold text-xs uppercase transition-all font-bold"
                >
                  {name}
                </button>
              ))}
      ...
      {isPendingCardConfirmation && (
        <div className="fixed inset-0 z-[600] bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-xs w-full space-y-6 text-center shadow-2xl animate-in zoom-in-95">
            ...
            <div>
              <h3 className="font-black text-xl tracking-tight font-bold">Kaartbetaling</h3>
            ...
            <div className="space-y-3">
              <button 
                ...
                className="w-full bg-emerald-500 text-white py-4 rounded-2xl font-bold uppercase text-xs shadow-lg active:scale-95 transition-all font-bold"
              >
                Betaling Geslaagd
              </button>
              <button 
                onClick={() => setIsPendingCardConfirmation(false)} 
                className="w-full bg-slate-100 text-slate-500 py-3 rounded-2xl font-bold uppercase text-xs hover:bg-slate-200 transition-colors font-bold"
              >
                Annuleren
              </button>
            </div>
      ...
      {isClosingSession && currentSession && (
        <div className="fixed inset-0 z-[600] bg-slate-950/70 backdrop-blur-sm flex items-center justify-center p-6">
          <div className="bg-white rounded-[2.5rem] p-7 max-w-xs w-full space-y-6 text-center shadow-2xl animate-in zoom-in-95">
            <h3 className="font-black text-xl tracking-tight font-bold">Shift Sluiten</h3>
            <div className="text-left space-y-2">
              <label className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block font-bold">Geteld Contant in Kassa (€)</label>
              <input 
                type="number" 
                step="0.01" 
                placeholder="0.00" 
            ...
            <div className="space-y-2">
              <button 
                ...
                className="w-full bg-rose-500 text-white py-4 rounded-2xl font-bold uppercase text-xs shadow-lg active:scale-95 transition-all font-bold"
              >
                Bevestig Sluiting
              </button>
              <button 
        ...
        <div className="fixed inset-0 z-[650] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-6">
          <div className="bg-white rounded-[2.5rem] p-6 max-w-sm w-full space-y-6 shadow-2xl max-h-[85vh] flex flex-col animate-in zoom-in-95">
            <div className="flex justify-between items-center shrink-0">
              <h3 className="font-black text-base tracking-tight font-bold">
            ...
            <button 
              onClick={() => { setPreviewTransaction(null); setPreviewSession(null); }} 
              className="w-full bg-slate-950 text-white py-4 rounded-2xl font-bold uppercase text-xs shadow-lg active:scale-95 transition-all shrink-0 font-bold"
            >
              Sluiten
            </button>
          </div>
      ...
      {showSuccess && (
        <div className="fixed inset-x-6 top-20 z-[700] bg-emerald-500 text-white p-4 rounded-2xl shadow-2xl flex items-center justify-center gap-3 animate-in slide-in-from-top duration-300">
          <CheckCircle size={20} />
          <span className="font-black text-sm uppercase tracking-wider font-bold">Transactie Voltooid!</span>
        </div>
      )}
    </div>
  );
