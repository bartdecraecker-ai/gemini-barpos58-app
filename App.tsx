...
export default function App() {
  ...
  useEffect(() => {
    // Debug/Requirement: Always launch with startpage (mode selection)
    setActiveMode(null);
    ...
    if (pinInput.length < 4) {
      ...
      if (newVal.length === 4) {
        if (newVal === (company.masterPassword || '1984')) {
          ...
          setLoginError(true);
          setTimeout(() => { setLoginError(false); setPinInput(''); }, 400);
        }
      }
    }
  };
  ...
  const totals = useMemo(() => {
    ...
    cart.forEach(i => {
      ...
      total += lineTotal;
      if (i.vatRate === 21) {
        vHigh += lineTotal - (lineTotal / 1.21);
      } else {
        v0 += lineTotal;
      }
    });
  ...
  const finalizePayment = async (method: PaymentMethod) => {
    ...
    if (btConnected) {
      try { await btPrinterService.printReceipt(tx, company); } catch(e) { console.warn("BT Print error", e); }
    }
  ...
  const closeSession = (counted: number) => {
    ...
    sessionTx.forEach(t => {
      t.items.forEach(i => { prodCounts[i.name] = (prodCounts[i.name] || 0) + i.quantity; });
    });
        ...
        <div className="max-w-xs w-full text-center space-y-8">
          ...
          <div className={`grid grid-cols-3 gap-4 p-4 rounded-3xl bg-slate-50 border transition-all ${loginError ? 'animate-shake border-red-200' : 'border-slate-100'}`}>
            {['1','2','3','4','5','6','7','8','9','C','0','X'].map(val => (
              ...
              }} className="h-16 rounded-2xl bg-white shadow-sm flex items-center justify-center font-bold text-xl active:scale-90 active:bg-slate-900 active:text-white transition-all">{val === 'X' ? <Delete size={20}/> : val}</button>
            ))}
          </div>
          <button onClick={() => { apiService.setActiveMode(null); setIsAuthenticated(false); setActiveMode(null); }} className="text-slate-400 text-xs font-bold uppercase tracking-widest py-2">Terug naar Selectie</button>
        </div>
      </div>
    );
  }
    ...
    <div className="fixed inset-0 flex flex-col bg-slate-50 overflow-hidden font-sans select-none">
      <header className="h-14 bg-slate-950 flex items-center justify-between px-6 shrink-0 z-50">
        <div className="flex items-center gap-4">
          <button onClick={() => btPrinterService.connect()} className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-4 text-white">
          ...
          <button onClick={() => { setActiveMode(null); setIsAuthenticated(false); }} className="text-white/20 hover:text-white transition-colors"><RefreshCcw size={14}/></button>
        </div>
      </header>

      <nav className="h-20 bg-white border-b flex items-center justify-around shrink-0 z-40 shadow-sm">
        <button onClick={() => setActiveTab('POS')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'POS' ? themeAccent + ' scale-105 font-bold' : 'text-slate-300'}`}><ShoppingBag size={24}/><span className="text-[9px] font-bold uppercase tracking-widest font-bold">Kassa</span></button>
        <button onClick={() => setActiveTab('REPORTS')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'REPORTS' ? themeAccent + ' scale-105 font-bold' : 'text-slate-300'}`}><BarChart3 size={24}/><span className="text-[9px] font-bold uppercase tracking-widest font-bold">Historiek</span></button>
        <button onClick={() => setActiveTab('SETTINGS')} className={`flex flex-col items-center gap-1 transition-all ${activeTab === 'SETTINGS' ? themeAccent + ' scale-105 font-bold' : 'text-slate-300'}`}><Settings size={24}/><span className="text-[9px] font-bold uppercase tracking-widest font-bold">Beheer</span></button>
      </nav>
      ...
      <main className="flex-1 overflow-hidden relative">
              ...
              <div className="flex-1 flex items-center justify-center p-6">
                <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl text-center max-w-sm w-full space-y-8 animate-in zoom-in-95">
                  ...
                  <div className="text-left space-y-2">
                    ...
                    <input type="number" step="0.01" value={startFloatAmount} onChange={e=>setStartFloatAmount(e.target.value)} className="w-full bg-slate-50 border-2 p-5 rounded-3xl font-bold text-3xl outline-none focus:border-indigo-400 transition-all text-center" />
                  </div>
                  <button onClick={() => {
                  ...
                  }} className="w-full bg-slate-950 text-white py-5 rounded-3xl font-bold uppercase shadow-xl hover:bg-slate-800 active:scale-95 transition-all font-bold">Start Shift</button>
                </div>
              </div>
            ) : (
              <>
                <div className="h-[35%] bg-white border-b flex flex-col overflow-y-auto p-4 space-y-2 relative shadow-inner custom-scrollbar">
                  ...
                  {cart.length === 0 ? <div className="h-full flex flex-col items-center justify-center opacity-10 py-10"><ShoppingBag size={48}/></div> : cart.map(item => (
            ...
            {currentSession && (
              <div className="bg-white p-7 rounded-[2.5rem] shadow-xl border-l-[10px] border-amber-500 flex justify-between items-center">
                <div>
                  ...
                  <div className="text-3xl font-black font-mono text-amber-500 font-bold">€{transactions.filter(t=>t.sessionId===currentSession.id).reduce((s,t)=>s+t.total,0).toFixed(2)}</div>
                </div>
                <button onClick={() => setIsClosingSession(true)} className="bg-rose-500 text-white px-6 py-4 rounded-2xl font-black text-xs uppercase shadow-lg active:scale-95 border-b-4 border-rose-700 font-bold">Sluiten</button>
              </div>
            )}
                ...
                <div key={s.id} className="bg-white p-6 rounded-[2rem] flex flex-col shadow-sm border border-slate-100 group transition-all">
                  <div className="flex justify-between items-start">
                    <div className="flex gap-4 items-center">
                      <div className="bg-slate-100 p-3.5 rounded-2xl text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-500 transition-all"><Calendar size={20}/></div>
                      <div>
                  ...
                  <div className="flex justify-end gap-3 mt-5 pt-4 border-t border-slate-50">
                    <button onClick={() => setPreviewSession(s)} className="flex items-center gap-2 px-4 py-2 bg-slate-50 text-slate-600 rounded-xl font-bold text-[10px] uppercase hover:bg-indigo-50 hover:text-indigo-600 transition-all font-bold">Rapport</button>
                    <button onClick={() => btPrinterService.printSessionReport(s, transactions.filter(t=>t.sessionId===s.id), company)} className="flex items-center gap-2 px-4 py-2 bg-slate-50 text-slate-600 rounded-xl font-bold text-[10px] uppercase hover:bg-emerald-50 hover:text-emerald-600 transition-all font-bold">Print</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'SETTINGS' && (
          <div className="h-full overflow-y-auto p-6 space-y-10 pb-32 custom-scrollbar">
               ...
               <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 space-y-6">
                 ...
                 <div className="pt-6 border-t space-y-4">
                   ...
                   <div className="space-y-2">
                     <input type="text" value={cloudConfig.syncId} onChange={e=>setCloudConfig({...cloudConfig, syncId: e.target.value})} className="w-full bg-slate-50 p-4 rounded-xl font-bold border-2 border-transparent focus:border-indigo-400 outline-none transition-all text-sm" placeholder="Sync ID..." />
                   </div>
                   <div className="grid grid-cols-2 gap-3">
            ...
            <div className="pt-8">
               <button onClick={() => { setActiveMode(null); setIsAuthenticated(false); }} className="w-full py-5 rounded-2xl bg-slate-900 text-white font-black uppercase text-[10px] tracking-widest shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2 font-bold font-bold"><LogOut size={16}/> Uitloggen</button>
            </div>
          </div>
        )}
      </main>
      ...
      {isPendingCardConfirmation && (
        <div className="fixed inset-0 z-[1500] bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in">
           <div className="bg-white rounded-[3rem] p-10 w-full max-w-sm text-center space-y-8 shadow-2xl border-8 border-sky-500/20">
              ...
              <div className="grid grid-cols-1 gap-3">
                <button onClick={() => finalizePayment(PaymentMethod.CARD)} className="w-full bg-sky-600 text-white py-6 rounded-[2rem] font-black uppercase text-sm shadow-2xl active:scale-95 border-b-4 border-sky-800 font-bold">Bevestig Betaling</button>
                <button onClick={() => setIsPendingCardConfirmation(false)} className="text-slate-400 text-[10px] font-black uppercase tracking-widest py-2 font-bold font-bold">Annuleren</button>
              </div>
           </div>
        </div>
      )}
      ...
      {editingProduct && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-6 animate-in fade-in">
           <div className="bg-white rounded-[3rem] p-10 w-full max-w-sm space-y-8 shadow-2xl">
              <h3 className="font-black text-2xl tracking-tighter flex items-center gap-3 font-bold"><Edit2 size={24} className="text-amber-500"/> Product Aanpassen</h3>
              ...
              <div className="flex gap-3 pt-4">
                 ...
                 }} className="flex-1 bg-emerald-500 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl active:scale-95 transition-all border-b-4 border-emerald-700 flex items-center justify-center gap-2 font-bold font-bold"><Save size={18}/> Opslaan</button>
                 <button onClick={() => setEditingProduct(null)} className="flex-1 bg-slate-100 text-slate-600 py-5 rounded-2xl font-black uppercase text-xs tracking-widest transition-all hover:bg-slate-200 active:scale-95 font-bold">Annuleer</button>
              </div>
           </div>
        </div>
      )}
      ...
      {showSalesmanSelection && (
        <div className="fixed inset-0 z-[1500] flex items-center justify-center bg-slate-950/80 backdrop-blur-md p-6 animate-in fade-in">
           <div className="bg-white rounded-[3rem] p-10 w-full max-w-sm space-y-8 shadow-2xl">
              ...
              <button onClick={() => setShowSalesmanSelection(false)} className="w-full bg-slate-950 text-white py-5 rounded-2xl font-bold uppercase active:scale-95 shadow-xl font-bold">Sluiten</button>
           </div>
        </div>
      )}
      ...
      {isClosingSession && (
        <div className="fixed inset-0 z-[1500] bg-slate-950/90 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in">
           <div className="bg-white rounded-[3rem] p-10 w-full max-w-sm text-center space-y-8 shadow-2xl">
              ...
              <button onClick={() => closeSession(parseFloat(endCashInput)||0)} className="w-full bg-rose-500 text-white py-6 rounded-[2rem] font-black uppercase text-sm shadow-2xl active:scale-95 border-b-4 border-rose-700 font-bold">Bevestig Sluiting</button>
              <button onClick={() => setIsClosingSession(false)} className="text-slate-400 text-[10px] font-black uppercase tracking-widest py-2 font-bold font-bold">Annuleren</button>
           </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[1200] bg-slate-950/95 flex flex-col items-center justify-center p-6 backdrop-blur-xl animate-in fade-in">
           <div className="bg-white rounded-[3rem] max-w-sm w-full overflow-hidden shadow-2xl border-8 border-white">
              ...
              <div className="p-8 border-t bg-white flex flex-col gap-3">
                <button onClick={() => { if(btConnected && previewTransaction) btPrinterService.printReceipt(previewTransaction, company); else window.print(); }} className="w-full bg-emerald-500 text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl active:scale-95 border-b-4 border-emerald-700 font-bold">Printen</button>
                <button onClick={() => { setPreviewTransaction(null); setPreviewSession(null); }} className="w-full bg-slate-950 text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest font-bold font-bold">Sluiten</button>
              </div>
           </div>
        </div>
      )}
