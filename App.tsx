...
export default function App() {
  ...
  // VERBETERD: Staat negatieve aantallen toe voor o.a. waarborg
  ...
  // VERBETERD: Berekent nu ook product Sales voor het dagrapport
  ...
  const processPayment = async (method: PaymentMethod) => {
    ...
    // BEVESTIGING: Vraag of er geprint moet worden
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-50">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
            ...
            <div className="flex-1 p-4 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 overflow-y-auto">
              {sortedProducts.map(p => (
                <button key={p.id} onClick={() => addToCart(p)} className={`${p.color} p-4 rounded-2xl shadow-sm hover:shadow-md transition-all active:scale-95 flex flex-col items-center justify-center min-h-[100px]`}>
                  <span className="font-bold text-slate-800 text-center leading-tight">{p.name}</span>
            ...
            <div className="w-full md:w-96 bg-white border-l flex flex-col shadow-xl">
              <div className="p-4 border-b flex justify-between items-center bg-slate-50">
                <h2 className="font-black flex items-center gap-2"><ShoppingBag size={20}/> TICKET</h2>
                <button onClick={() => setShowRecentTransactions(true)} className="p-2 hover:bg-slate-200 rounded-full"><History size={20}/></button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-2">
                {cart.map(item => (
                  <div key={item.id} className="flex items-center justify-between p-2 border-b border-slate-50">
                    ...
                    <span className="flex-1 px-3 text-sm font-medium truncate">{item.name}</span>
              ...
              <div className="p-4 bg-slate-900 text-white rounded-t-3xl">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => processPayment(PaymentMethod.CASH)} className="bg-green-600 p-4 rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95 transition-transform"><Banknote /> CASH</button>
                  <button onClick={() => processPayment(PaymentMethod.CARD)} className="bg-blue-600 p-4 rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95 transition-transform"><CreditCard /> CARD</button>
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Andere tabs (Reports/Settings) hier implementeren zoals in origineel... */}
      </main>
